...
import {
  CORRUPTION_CRITICAL_VALUE,
  CORRUPTION_MAX_VALUE,
  CORRUPTION_STABLE_VALUE,
  CORRUPTION_UNSTABLE_VALUE,
  NO_CORRUPTION_VALUE,
} from "../shared/consts";
import { CorruptionStage } from "../shared/enums/corruptionStage";

const getCorruptionStage = (corruptionValue: number): CorruptionStage => {
  return corruptionValue < CORRUPTION_STABLE_VALUE
    ? CorruptionStage.Stable
    : corruptionValue < CORRUPTION_UNSTABLE_VALUE
      ? CorruptionStage.Unstable
      : corruptionValue < CORRUPTION_CRITICAL_VALUE
